...
void ReaderManager::addReader(Chunk *chunk) {
  readers.emplace_back(chunk, [](Reader *reader) {
    ...
    std::cout << "Starting new thread " << std::this_thread::get_id() << " reading from "
              << filename << "..." << std::endl;
    {
...
ReaderManager::~ReaderManager() { assert(readers.empty()); }
  ...
  for (Reader &reader : readers) {
    ...
    while (reader.queue.try_dequeue(msg)) {
      if (std::holds_alternative<Event>(msg)) {
        ...
        std::cerr << "Error reading " << reader.chunk->getFilename() << ": "
                  << std::get<Error>(msg).msg << std::endl;
      } else {
        assert(false);
      }
