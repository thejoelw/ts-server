...
template <typename Consumer> class Decompressor : public Consumer {
public:
  ...
  std::size_t getPrefferedSize() const { return ZSTD_DStreamInSize(); }
  ...
  void onData(const char *data, std::size_t size) {
    ...
    while (inBuf.pos < inBuf.size) {
      ...
      if (ZSTD_isError(res)) {
        static_cast<Consumer *>(this)->onError(
            std::string("Zstd error: ") + ZSTD_getErrorName(res)
        );
        ended = true;
        break;
      }
      ...
      if (outBuf.pos != prevPos) {
        static_cast<Consumer *>(this)->onData(
            static_cast<char *>(outBuf.dst) + prevPos, outBuf.pos - prevPos
        );
      }
  ...
  template <typename MemType> MemType onBestow(MemType mem) {
    // Don't need the memory any more (already consumed by zstd), so can immediately give back to
    // producer for re-use.
