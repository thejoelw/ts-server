...
template <typename Consumer> class Decoder : public Consumer {
public:
    ...
    while (size != 0) {
      if (remainingSize != 0) {
        if (remainingSize <= size) {
          if (catMsgs.empty()) {
            static_cast<Consumer *>(this)->onEvent(
                Event(Instant::fromUint64(timeRegister), data, remainingSize)
            );
          } else {
            catMsgs.insert(catMsgs.cend(), data, data + remainingSize);
            assert(catMsgs.size() == sizeRegister);
            static_cast<Consumer *>(this)->onEvent(
                Event(Instant::fromUint64(timeRegister), catMsgs.data(), catMsgs.size())
            );
