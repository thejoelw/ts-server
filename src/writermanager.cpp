...
void WriterManager::open(const std::string &filename) {
  ...
  thread = std::thread(
      [](const std::string &filename, moodycamel::BlockingReaderWriterQueue<QueueMessage> *queue) {
        {
          std::cout << "Starting new thread " << std::this_thread::get_id() << " writing to "
                    << filename << "..." << std::endl;
          ...
          {
            Encoder<Buffer<
                Compressor<Chunker<FileWriter, 64 * 1024 * 1024 - 16>>,
                64 * 1024 * 1024 - 16>>
                pipe(std::ref(filename));

            std::chrono::steady_clock::time_point flushTime =
                std::chrono::steady_clock::now() + defaultFlushDelay;

            QueueMessage msg;
            while (true) {
              std::chrono::steady_clock::duration wait =
                  flushTime - std::chrono::steady_clock::now();
              if (wait.count() > 0 && queue->wait_dequeue_timed(msg, wait)) {
                ...
                std::cout << "Flushing thread " << std::this_thread::get_id() << " (processed "
                          << messageCount << " messages)..." << std::endl;
                pipe.onFlush([]() {});
                std::cout << "Flushed thread " << std::this_thread::get_id() << " (processed "
                          << messageCount << " messages)" << std::endl;
                flushTime = std::chrono::steady_clock::now() + defaultFlushDelay;
              }
            }
          }
          std::cout << "Ended writer thread " << std::this_thread::get_id() << " (processed "
                    << messageCount << " messages and " << eventCount << " events)" << std::endl;
        }
      },
      filename,
      queue
